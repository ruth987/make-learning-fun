...
const HeroCard2: React.FC<GoalCardProps> = ({ image, goal }) => {
  return (
    <div className="flex flex-col items-center justify-center bg-purple-100 rounded-lg p-4 shadow hover:shadow-2xl transition-shadow duration-300 h-44 px-10 ">
      ...
      <div className="aspect-w-3 aspect-h-2 w-full">
        <img src={image} alt="Goal" className="object-cover rounded-t-lg h-52" />
      </div>
      <div className="flex-grow flex items-center justify-center">
        <p className="text-center text-purple-900 text-xl shadow-md">{goal}</p>
      </div>
      </div>
    </div>
  );
